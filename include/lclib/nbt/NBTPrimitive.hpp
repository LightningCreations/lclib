...
namespace nbt{
    class LIBLCAPI NBTPrimitive:public NBTTagBase{
    public:
        ...
        virtual bool getBoolean()const noexcept(true);
    ...
    namespace detail{
        template<typename T,uint8_t tag_type> class NBTTagPrimitive:public NBTPrimitive
			,private RelOps<NBTTagPrimitive>,private StrictOrder<NBTTagPrimitive>{
        private:
            T value;
        public:
			...
			bool operator< (const NBTTagPrimitive& p)const{
