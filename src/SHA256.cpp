...
const unsigned k[] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
...
void SHA256(const char* in,size_t size,char (&out)[32]){
	...
	for(size_t Q = 0;Q<chunks;Q++){
		...
		for(size_t  i = 16;i<64;i++){
			unsigned s0 = rrotate(w[i-15],7)^rrotate(w[i-15],18)^(w[i-15]<<3);
			unsigned s1 = rrotate(w[i-2],17)^rrotate(w[i-2],19)^(w[i-2]<<10);
		...
		for(size_t q = 0;q<64;q++){
			unsigned S1 = rrotate(e,6)^rrotate(e,11)^rrotate(e,25);
			unsigned ch = (e&f)^(~e&g);
			unsigned tmp1 = i + S1 + ch + k[q] + w[q];
			unsigned S0 = rrotate(a,2)^rrotate(a,13)^rrotate(a,22);
