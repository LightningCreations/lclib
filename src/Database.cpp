...
void DirectStatement::setLong(int,long){}
...
static std::shared_mutex providersLock;

static std::vector<ConnectionProvider*> providers;
...
Connection& SQLPreparedStatement::getConnection(){
	return underlying->getConnection();
}
...
Transaction::Transaction(DBConnection& conn):conn(&conn){
