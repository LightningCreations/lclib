...
protected:
    ...
    virtual void do_nextBytes(uint8_t*,std::size_t);
...
public:
    ...
    */
    void nextBytes(uint8_t*,size_t);
    /*
        Fills an array with its size known at compile time with a bunch of psuedo-random bytes.
        This method does not particpate in overload resolution if byte does not satisfy the requires for Byte, or is const-qualified.
    */
	template<typename byte, size_t size> std::enable_if_t<is_byte_v<byte>&&!std::is_const_v<byte>> nextBytes(array<byte,size>& bytes){
        this->nextBytes(reinterpret_cast<uint8_t*>(bytes.data()),size);
    }
	/**
	 * Fills an array with its size known at compile time with a bunch of pseudo-random bytes.
	 * This method does not particpate in overload resolution if byte does not satisfy the requires for Byte, or is const-qualified.
	 */
	template<typename byte,size_t N> std::enable_if_t<is_byte_v<byte>&&!std::is_const_v<byte>> nextBytes(byte(&arr)[N]){
		this->nextBytes(reinterpret_cast<uint8_t*>(arr),N);
	}
	/**
	 * Fills an contiguous memory region specified by the span passed in.
	 * This method does not particpate in overload resolution if byte does not satisfy the requires for Byte, or is const-qualified.
	 */
	template<typename byte> std::enable_if_t<is_byte_v<byte>&&!std::is_const_v<byte>> nextBytes(span<byte> s){
		this->nextBytes(reinterpret_cast<uint8_t*>(s.data()),s.data());
	}
	/**
	...
	/*
	 * Updates the Pseudo-random Number generator and returns the bits generated by the call to next()
	 * Provided to satisfy UniformRandomBitsGenerator, which is required in LCLib-C++ 1.3
	 */
	uint32_t operator()();
